...
func cmd() *cobra.Command {
	...
	c := &cobra.Command{
		Use:   "dead-method-checker",
		Short: "Find unused methods in Go structs",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			unusedMethods, err := search(cmd.Context(), *targetPkgPath, *targetStructName)
			if err != nil {
				return err
			}
	...
	targetPkgPath = c.Flags().String("pkg", "", "[REQUIRED] Path of the package to check for unused methods")
	targetStructName = c.Flags().String("struct", "", "[REQUIRED] Name of the struct to check for unused methods")
	return c
}
